...
반환되는 동전이 "최소"가 되는 자판기 구현

- 자판기가 보유하고 있는 금액을 입력하고 금액에 맞게 동전을 무작위로 생성
- 잔돈을 돌려줄 때 "자판기가 현재 보유한 동전의 최소 개수"의 동전으로 잔돈을 돌려준다.
- 지폐를 잔돈으로 바꾸는 경우는 X
- 상품명 가격, 수량을 입력하여 상품을 추가할 수 있음
    - 상품 가격은 100원부터 시작하여 반드시 10원으로 나누어 떨어져야함
- 사용자가 투입한 금액으로 상품을 구매할 수 있음
- 남은 금액이 상품의 최저 가격보다 작거나, 모든 상품이 소진되는 경우 바로 잔돈을 돌려준다.
- 잔돈을 반환할 수 없는 경우 잔돈으로 반환할 수 있는 금액만 반환
    - 반횐되지 않은 금액은 자판기에 남음.
- 사용자가 잘못된 값 입력 -> "[ERROR]"로 시작하는 에러메시지 출력 -> 그 부분 재입력
...
1. 자판기가 보유하고 있는 금액
2. 상품명, 가격, 수량은 ','로, 개별 상품은 []로 묶어 ;으로 구분
   [콜라,1500,20];[사이다,1000,10]
3. 투입 금액
4. 구매할 상품명
...
2. 투입 금액 - 자판기가 사용자로부터 받은 금액 중 잔액
   투입 금액: 3000원
...
- 제시된 Coin 클래스를 활용한다.
- 이 때 Coin 클래스의 amount의 접근제어자 private은 변경 X
- Randoms, Console 라이브러리 활용
